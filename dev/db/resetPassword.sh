...
resetCommand="mysql $databaseConnectionString -e 'UPDATE \`user\` SET \`password\` = \"$newPasswordHash\" WHERE \`email\` = \"$email\"'";
...
if [[ "$DRY_RUN" == "true" ]]; then
  ...
  if eval "$resetCommand"; then
    echo -e "Password hash updated to: ${GREEN}$newPasswordHash${NC}";
