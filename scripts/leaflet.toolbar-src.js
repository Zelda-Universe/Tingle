...
L.Toolbar = (L.Layer || L.Class).extend({
	...
	appendToContainer: function(container) {
      /** 001 - BEGIN **/
      // CODE CHANGE TO WORK WITH LEAFLET 1.0RC
      if (this.constructor.baseClass == undefined) {
         this.constructor.baseClass = "leaflet-toolbar";
      }
      /** 001 - END **/
...
L.ToolbarAction = L.Handler.extend({
	...
	initialize: function(options) {
      ...
      /** 001 - BEGIN **/
      // Hides the subToolbar whenever maps moves
      map.on('movestart', function(e) {
         this.disable();
         toolbar.lastClicked = -1;
         this.options.subToolbar.visible=false
      }, this);
      /** 001 - END **/
	},
   ...
   show: function(visible) {
      console.debug(this.options.subToolbar);
      this.options.subToolbar.visible = visible;
   },
   /** 001 - END**/
	...
	_addSubToolbar: function(toolbar, container, args) {
		...
		if (subToolbar.options.actions.length > 0) {
            ...
            /** 001 - BEGIN **/
            // Toogle On/Off
            if (toolbar.lastClicked == subToolbar._leaflet_id) {
               if (!subToolbar.visible) {
                  subToolbar._show();
                  subToolbar.visible = true;
                  toolbar.lastClicked = subToolbar._leaflet_id;
               } else {
                  subToolbar._hide();
